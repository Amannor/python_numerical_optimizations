...
def f_b_i(x):
    Q = np.array([[1.,0],[0,1.]])
    f_val = eval_quad(Q, x)
    grad_vector_val = np.array([2*x[0], 2*x[1]])
    return f_val, grad_vector_val

def f_b_ii(x):
    Q = np.array([[5.,0],[0,1.]])
    f_val = eval_quad(Q, x)
    grad_vector_val = np.array([5*2*x[0], 2*x[1]])
    return f_val, grad_vector_val
...
def f_b_iii(x):
    ...
    Q = q1.dot(q2).dot(q3) # Q is np.array([[4, -math.sqrt(3.)],[-math.sqrt(3.)/2., 2]]) i.e. f = 4x[0]^2 - sqrt(3)x[0]^2 - - sqrt(3)x[1]^2 +2x[1]^2
    f_val = eval_quad(Q, x)
    grad_vector_val = np.array([8*x[0] - 2*math.sqrt(3)*x[0], 4*x[1]-2*math.sqrt(3)*x[1]])
    return f_val, grad_vector_val
